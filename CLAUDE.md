...
- `MITgcm_c69m/mods_tapenade_hooks/` — the Tapenade output hooks (the `ADJ*`, `ADJetan` and `G_J*` dumps and the adjoint-mode switches) as one `-mods` directory shared by every adjoint build of both setups (since 2026-09-07), and at the same time the upstream proposal, file for file: seven hook files (five tree files with lines added, each under the tree file's own name, and two new files), a README mapping each to its place in the tree, `check_against_tree.sh` and the generated `patches/`; since 2026-09-12 also an eighth file, `gad_advection.F`, the one-guard change that lets `useApproxAdvectionInAdMode` act under Tapenade (patch 0003 of three), so every adjoint build of every setup honours that switch. The build body lists it first in `-mods` and hands its `flow_tap` to Tapenade as a second `-ext`. See "The ADJ* dump hook" under Build.
...
./scripts/build_tapAdj.sh        # -> build_tapAdj_ckpAll/mitgcmuv_tap_adj (a symlink: see below)
```

Both setups name scripts action-first (`build_*`, `submit_*`), and the `rawTapenade` control builds are retired in both — raw Tapenade output *is* the working configuration. **In DINO, since 2026-09-02, `scripts/build_tapAdj.sh` and `scripts/submit_tapAdj.sh` are symlinks** to the default adjoint pair beside them (`_ckpAll` since 2026-09-12, `_approxAdv` from 2026-09-10, `_nocheckpoint` from 2026-09-02); every real DINO adjoint script carries a token saying what it builds. The tokens name only what is compiled differently: which adjoint a run is (GM/Redi kept out of the adjoint sweep, scheme 30 in place of scheme 33 there) is set by `data.autodiff`, whose switches every DINO adjoint build honours since 2026-09-12:

| DINO pair (build / submit) | Build directory | What |
| --- | --- | --- |
| `build_tapAdj.sh` → `_ckpAll`, `submit_tapAdj.sh` → `_ckpAll` | `build_tapAdj_ckpAll/` | **the default since 2026-09-12** (`_approxAdv` from 2026-09-10, `_nocheckpoint` from 2026-09-02): symlinks, repointed when the default changes |
| `build_tapAdj_nocheckpoint.sh` / `submit_tapAdj_nocheckpoint.sh` | `build_tapAdj_nocheckpoint/` | profile-guided `-nocheckpoint` list (28 routines in split mode since 2026-09-12, 33 from 2026-09-02): the `ckpAll` adjoint bit for bit, with less recomputation. **The default from 2026-09-02 to 2026-09-10.** The 2026-09-02 list split `thermodynamics`, `do_oceanic_phys` and `dynamics`, which are recorded before the adjoint-mode switches act and read what they switch, so the submit body refused it with the live namelist from 2026-09-11; the list of 2026-09-12 (profile 31268) keeps those three checkpointed, `post_build_checks` verifies that with `tools/tapenade_profiling/check_nocheckpoint_switches.py` and records `nocheckpoint_switch_free=yes`, and under the live switches the build reproduces the checkpoint-everything adjoint bitwise at 1.41× (31276 vs 31269, 30 d; 31277 vs 31259, 5 d; both references from the `approxAdv` build). Since 2026-09-12 it also compiles `code_tap/gad_implicit_r.F` (rebuilt: 31285 vs 31277 identical) |
| `build_tapAdj_ckpAll.sh` / `submit_tapAdj_ckpAll.sh` | `build_tapAdj_ckpAll/` | **the default since 2026-09-12**: every call checkpointed (Tapenade's default), so an adjoint-mode switch reaches every recomputation. It compiles the approximate-advection replacement for all tracer fluxes (`gad_advection.F` from the shared hooks directory, `code_tap/gad_implicit_r.F`) and gives the former `approxAdv` adjoint bit for bit (31281 vs 31259 on the live namelist; 31282 vs 31279 with implicit vertical advection). **Was `build_tapAdj.sh` / `build_tapAdj/` until 2026-09-02** — every run up to job 31055 that is not named `nocheckpoint`/`tapProfile`/`adjViscBoost` came from it. Also the profiler's base, the fallback if a configuration change invalidates the list, and the timing baseline |
| `build_tapAdj_adjVisc.sh` / `submit_tapAdj_adjVisc.sh` | `build_tapAdj_adjVisc/` | `ckpAll` **plus** the ASTE `inAd*` sources, compiled from `code_tap/variants/adjointViscosity/` as a second `-mods` directory; its submit definition adds the boost's values to the staged `data.autodiff` (since 2026-09-12; 31286 vs 31271 identical). Deliberately without the list: under the boost, split mode changes the adjoint at order one (run 31056 vs 31025, 2026-09-02 — see the `adjVisc` bullet under "Run") |
...
| `build_tapAdj_hooksInTree.sh` / `submit_tapAdj_hooksInTree.sh` | `build_tapAdj_hooksInTree/` | validation of the **in-tree** form of the hooks (2026-09-05, simplified 2026-09-07): builds against `$HOME/MITgcm_c69m_tapenade_hooks/MITgcm` (a git copy of checkpoint69m + 5 commits on branch `tapenade-hooks`, the fifth, `95ee6a696` of 2026-09-12, the `gad_advection.F` guard; the definition asserts the tree's copies of the files of `mods_tapenade_hooks/` byte-identical to them; `IMPACTS_HOOKS_TREE` overrides the path) with the shared directory left out (`HOOKS_MODS=""`) and the `-nocheckpoint` list and switch check of `build_tapAdj_nocheckpoint.sh`; run 31107 bitwise identical to 31101, and after the rebuild of 2026-09-12 run 31284 identical to 31278. The only build whose `build_info.txt` carries `mitgcm_root=`; `MITGCM_TREE` in the definition is what repoints the build body |

**From 2026-09-10 to 2026-09-12 the default was `build_tapAdj_approxAdv.sh` / `submit_tapAdj_approxAdv.sh`** (build directory `build_tapAdj_approxAdv/`, run token `tapAdj_ckpAll_approxAdv`): the `ckpAll` build plus `code_tap/variants/approxAdvection/` (from 2026-09-09) as a second `-mods` directory, holding `gad_advection.F` with the guard of its `useApproxAdvectionInAdMode` block widened from `ALLOW_AUTODIFF_TAMC` to `ALLOW_AUTODIFF`, and `gad_implicit_r.F` with the same replacement added for implicit vertical advection, which the tree lacks under any AD tool; with the switch off it reproduced the `ckpAll` adjoint digit for digit (31179 vs 31178). On 2026-09-12 the first file moved into `mods_tapenade_hooks/` and the second into `code_tap/` (listed in `code_tap/TREE_BASE.txt`), so every DINO adjoint build compiles both and the pair no longer differed from `ckpAll`: it was deleted, the default symlinks were repointed to `ckpAll`, and `build_tapAdj_approxAdv/` was removed (run 31279 keeps its executable). Its runs keep `tapAdj_ckpAll_approxAdv` in their names; a run of the live namelist from any DINO adjoint build now carries `_approxAdv` among its settings tokens instead.

SOMA's `build_tapAdj.sh` / `submit_tapAdj.sh` are real files and still checkpoint every call — no SOMA profile exists yet — so the bare name means different things in the two setups (SOMA's run token is nevertheless `tapAdj_ckpAll`, the same vocabulary, so a tuned SOMA build can be told apart by name when one exists). Optfiles come from `tools/machine_env.sh`, so nothing needs exporting. The build body does the same steps in the same order for every definition, and never copies anything into `code/` or `code_tap/` (since 2026-09-02 a build leaves the working tree clean): `make CLEAN`, run **stock** `genmake2` with the definition's `-mods` list and, for an adjoint, `MITgcm_c69m/mods_tapenade_hooks/` put first in that list, `-tap`, the tree's stock `-adof` and its `TAP_EXTRA` string with `-ext` of the shared directory's `flow_tap` appended (the `_nocheckpoint` definition's `pre_configure` reads `code_tap/tap_nocheckpoint.txt` into `-tap_extra "-nocheckpoint \"<list>\""`, the profiler's sets `-tap_extra "-profile"`; the `adjVisc` definition lists `../code_tap/variants/adjointViscosity` ahead of `../code_tap` and its `post_build_checks` asserts the compiled `autodiff_*.f` came from the variant), `make depend`, `make -j 8 [tap_adj]`, assert every generated hook call's argument count (`check_gen_call`, over the `HOOK_CHECKS` list in `scripts/setup_params.sh` — the same seven hooks in both setups since 2026-09-07), the five `ADJ*` dump calls in the compiled `dummy_tap.f`, that the hook sources link into the shared directory and that each compiled source named in the setup's `COMPILED_NAME_CHECKS` carries its name (DINO, since 2026-09-12: `useApproxAdvectionInAdMode` in `gad_advection.f`, `gad_implicit_r.f` and their `_b.f`), and — last, so it can only describe a verified executable — write `build_info.txt` into the build directory (script, `TAP_EXTRA`, the list, commit, `exe_md5`, a `run_token` such as `tapAdj_ckpAll_adjVisc` and, for an adjoint since 2026-09-12, `approx_advection_implicit_vertical=yes|no`; since 2026-09-05 forward builds write one too, with `run_token=frd`). The submit body refuses an executable that has no record, whose checksum does not match the record's `exe_md5` (since 2026-09-03; the earlier mtime test fired spuriously on the NFS home and survives only as the fallback for records without that line), or whose `run_token` is not the one the submit definition names in `EXPECT_RUN_TOKEN` (since 2026-09-05 the build/submit pairing is enforced, not just documented), copies the record into the run directory, and takes the run directory's name from `run_token` (see "Where the output lands"). Once the namelists are staged it also refuses, in DINO, an adjoint namelist without the adjoint-mode switches (`check_staged_namelists`, since 2026-09-12; see "Run"), a `-nocheckpoint` build whose record does not say its list is safe under the switches the namelist throws (`nocheckpoint_switch_free=yes`), and implicit vertical advection with the scheme switch on in a build whose record lacks `approx_advection_implicit_vertical=yes`.
...
- **Every copy of a tree file in a variant directory names its origin** (since 2026-09-12). `TREE_BASE.txt` beside the copies lists, for each, the tree file it shadows and the git blob of that file it was derived from; `tools/check_variant_shadows.sh`, run by `tools/pre_push_check.sh`, fails when a tree file changes under its copy, when a copy is not listed, or when a copy has become identical to the tree. DINO's `code_tap/` keeps one too, for the tree files it carries whole (`gad_implicit_r.F`, `the_main_loop.F`, `cost_atlantic_heat.F`); beside the setup's own headers there, only the listed files are checked. A checkpoint upgrade therefore cannot silently compile the old checkpoint's text through a variant: re-derive the copy, then `--record <dir>`; `--tree=<newer tree>` shows beforehand which copies an upgrade would invalidate. `tools/tapenade_profiling/mods_profile/` carries a manifest too.
...
**Both setups, one shared directory since 2026-09-07: the hooks are `MITgcm_c69m/mods_tapenade_hooks/`, a `-mods` directory that is also the upstream proposal.** The pattern covers the `ADJ*` dump (a wrapper `DUMMY_IN_STEPPING_TAP` called beside `DUMMY_IN_STEPPING`), the `ADJetan` dump (`DUMMY_FOR_ETAN_TAP` beside `DUMMY_FOR_ETAN` in `INTEGR_CONTINUITY`, a separate hook because the free-surface adjoint is half a time step out of phase with the rest) and the two adjoint-mode switches (`AUTODIFF_INADMODE_SET_TAP`/`UNSET_TAP` beside the stock switches; their `_B` bodies call the TAF-named `ADAUTODIFF_INADMODE_SET`/`UNSET`, which apply/revert the `inAd*` adjVisc parameters at the start/end of every backward step and were dead code under Tapenade before 2026-08-31, so adjVisc never actually boosted). The directory's seven hook files, every one under the name it has or would have in the tree (an eighth file, `gad_advection.F`, is not a hook; last bullet):
...
- `stubs_tap_adj.F` — shadow of the `pkg/tapenade` file with the five `ADEXCH_*` stubs implemented (the seam fix of 2026-08-31, see below); one of the three files that remove lines from their tree counterpart (with `dummy_tap.F` and `gad_advection.F`), and the first of the three patches.
- `gad_advection.F` (since 2026-09-12, commit `5ab5e22`) — shadow of the `pkg/generic_advdiff` file with one CPP guard widened from `ALLOW_AUTODIFF_TAMC` to `ALLOW_AUTODIFF`, around the block that replaces the flux-limited DST3 (scheme 33) by the unlimited one (30) while `inAdMode` is true (`useApproxAdvectionInAdMode`). Not a hook: the block was compiled out of every Tapenade build, so the switch did nothing; a TAF build and a forward build preprocess the file to the tree's text. It is the third patch because only the mode-switch hooks set `inAdMode` under Tapenade. It covers the horizontal and explicit vertical fluxes; implicit vertical advection is `gad_implicit_r.F`, which DINO carries in its own `code_tap/`.

`check_against_tree.sh` in that directory verifies the shape (new file, or tree file plus added lines; `stubs_tap_adj.F`, `dummy_tap.F` and, since 2026-09-12, `gad_advection.F` are the three declared exceptions that also remove lines), regenerates `patches/` and checks that they apply to the vendored tree (`git apply --check`); `tools/pre_push_check.sh` runs its `--check` form. The four `pkg/autodiff` hooks and their TAF adjoints (`addummy_in_stepping.F`, `addummy_for_etan.F`) compile untouched from the vendored tree, as dead code under Tapenade, exactly as in an upstream Tapenade verification build. A stock verification experiment can use the directory through a `genmake_local` in its `build/` (`MODS="../code_tap <dir>"`, `TAP_EXTRA="-ext <dir>/flow_tap"`; genmake2 reads it after the command line) — never leave one in the vendored tree, upstream un-ignores that file.
...
### Tapenade profiling and `-nocheckpoint` — wired in 2026-09-01, list re-derived 2026-09-12 (DINO)
...
| `build_tapAdj_nocheckpoint.sh` / `submit_tapAdj_nocheckpoint.sh` | **the default adjoint from 2026-09-02 to 2026-09-10** (`build_tapAdj.sh` / `submit_tapAdj.sh` were symlinks to this pair; since 2026-09-12 they point at the checkpoint-everything pair, `build_tapAdj_ckpAll.sh`). `-tap_extra "-nocheckpoint \"<list>\""`, the list read from `code_tap/tap_nocheckpoint.txt` (28 routines since 2026-09-12, each annotated with the gain that put it there). Tapenade differentiates those in split `_FWD`/`_BWD` mode instead of checkpointing them; the build fails unless every listed name produced a `_FWD` routine, so the list cannot silently drift, and records whether the list is safe under the adjoint-mode switches (`nocheckpoint_switch_free=`). |

What the 2026-09-01 profile said (run 31053, 30 d, rank 0, 809 s of adjoint): checkpointing costs 363 s of CPU per process — 45 % of the run — spread over 156 call sites / 116 callees, and almost all of it in routines whose split mode is *memory-neutral or a memory gain* (`timestep` 75 s and −31 MB, `forward_step` 71 s, `grad_sigma` 37 s, `mom_vecinv` 22 s, `calc_phi_hyd` 21 s and −8.6 MB, `thermodynamics` 18 s, …); the entries that do cost memory sum to ~54 MB against a 923 MB peak tape per process. The per-level routines dominate because joint-mode Tapenade snapshots whole 3-D arrays on each per-level call. **Validation (30 d, same node): run 31054 vs plain 31052 — `fc` identical, all 32 `adxx_*` and all 73 `ADJ*` files bitwise identical, wall time 8:47 vs 13:13 (1.50×, −33.5 %).** At 5 years (31055 vs 31039): again bitwise identical (`fc`, 32 `adxx_*`, 4 393 `ADJ*`), 9:35:58 vs 14:05:45 (1.47×, 4.5 h saved; forward sweep unchanged at 0.84 h, reverse sweep 8.76 vs 13.24 h = 1.51×). On the full κ_v ensemble (2026-09-02/03, jobs 31060–31067 vs 31039–31046): all eight 5-yr adjoints bitwise identical — `fc`, 32 `adxx_*`, 4 393 `ADJ*` and the `%MON` stream per pair, the four blown-up members included — at 1.45–1.65× per run (reverse sweep 1.54×, forward sweep unchanged), 37.8 h saved over the eight; report in `analyses/DINO_1deg/adjoint/tapenade_profiling/compare_5yr_kappa_ensemble_ckpAll_vs_nocheckpoint.md`. The c69f-era 64-routine list (`tools/tapenade_profiling/nocheckpoint_routines.txt`) shares only 8 routines with the 2026-09-02 one (7 with the current) and none of the top twelve; under this profile it would have recovered 21 s of the 363 s — re-profiling was not optional.

**Re-derived on 2026-09-12** for the current configuration: profile 31268 of the live namelists (31 d; 411 s of checkpointing on rank 0, 837 MB peak tape — the four KPP fields gone from each of the 98 binomial snapshots account for 85 MB of the drop from 923). The 2026-09-02 list no longer built (`kpp_calc_dummy`) and was unsafe under the switches. **Where an adjoint-mode switch acts:** `FORWARD_STEP` calls `AUTODIFF_INADMODE_SET_TAP` last, so its `_B` applies the switches at the start of the step's reverse sweep; a checkpointed routine's primal is re-run, recording, inside its `_B` after that, while a split routine is recorded wherever its caller records — before the switch when reached from `FORWARD_STEP` through split routines only. So a list is safe under a switch if no such routine reads a switched variable (`inAdMode`, the `use<PKG>` flags, `viscFacAdj`) anywhere below it; `tools/tapenade_profiling/check_nocheckpoint_switches.py` checks that statically (dropping the flags of packages not compiled), `--filter` derives a safe list, and the two list builds record the result. The new list is the profile's 34 callees ≥ 1 s minus 3 externals minus `thermodynamics`, `do_oceanic_phys` and `dynamics` — 28 routines, 354 of 411 s — and it splits `forward_step` (recorded before the switch in either mode) and `temp_integrate`, `salt_integrate`, `gad_advection`, `gad_calc_rhs`, `mom_vecinv` and `mom_calc_visc`, which read switched variables but sit below a checkpointed caller. **Validation under the live switches:** 31276 vs 31269 (30 d, `stability_study/from180yrPk_viscRef_ReMax2_gmFwd`, same node) EQUIVALENT in all 786 sensitivity files, `fc` and `%MON`, 8:24 vs 11:53 (1.41×); 31277 vs 31259 (5 d, live namelist) EQUIVALENT; 31278 (`hooksInTree`) vs 31277 EQUIVALENT; rebuilt after the build consolidation later that day, 31285 vs 31277 and 31284 vs 31278 identical. `tools/tapenade_profiling/README.md` section 4 has the argument; `analyses/DINO_1deg/adjoint/tapenade_profiling/` the records.
...
../../../tools/submit.sh scripts/submit_tapAdj.sh     # adjoint, committed default (DINO: symlink to submit_tapAdj_ckpAll.sh since 2026-09-12)
...
- **The DINO adjoint pickup follows the namelist (since 2026-09-12).** `stage_pickups` in every DINO adjoint submit definition calls `link_pickup` (`scripts/setup_params.sh`), which links `pickup.<nIter0>.{data,meta}` for the staged namelist's `nIter0` (nothing for a run from rest; `IMPACTS_PICKUP_ITER` overrides the iteration) from `IMPACTS_PICKUP_RUN_DIR` or, when that is unset, from the production spin-up 31203, and refuses that default for a namelist whose derived settings (`run_suffix_from_namelist`) do not contain `viscRef_ReMax2`: a `visc2x` variant has to name the 2× spin-up 30983, a κ member its own leg. Until 2026-09-12 each definition hard-coded 30983's `pickup.0003162240` (overridable since 2026-09-09), which the live namelist had not matched since 2026-09-09. A kappa member's own pickup and a mid-window restart of an adjoint run (which keeps monthly pickups, `pChkptFreq` in `input_tap/data`) are the uses of the override so far. DINO's forward definition and SOMA's and the gyre's still link a fixed pickup.
- **`nIter0` is *not* one of the auto-patched parameters.** It is baked into whichever `data_<tag>` the `test_cases` string selects (`from_rest` → `0`, `from50yrPk` → `878400`, `from70yrPk` → `1229760`, `from180yrPk` → `3162240`). DINO's adjoint definitions derive the pickup from it (previous bullet); DINO's forward definition and SOMA's still link a fixed `pickup.<iteration>`, so for those, changing `test_cases` to a different `from*Pk` tag without the matching `IMPACTS_PICKUP_ITER` gets a run that cannot find its pickup. Changing the duration is safe.
- **A DINO adjoint namelist must set its adjoint-mode switches (since 2026-09-12).** `check_staged_namelists` in `scripts/setup_params.sh`, which the submit body calls once the namelists and the definition's `stage_extra` are in the run directory, refuses a staged `data.autodiff` without `useGMRediInAdMode=.FALSE.` while `data.pkg` turns GM/Redi on, or without `useApproxAdvectionInAdMode=.TRUE.` while `data` sets `tempAdvScheme` or `saltAdvScheme` to 33. The exact adjoint of either blows up (31236; 31166 against 31167), and since every DINO adjoint build honours both switches, no build name can say which adjoint a run is. A record that needs the exact adjoint (`stability_study/*_gmOn`, `grdchk_repair/*_approxAdvOff`) is submitted with `IMPACTS_ALLOW_EXACT_ADJOINT=1`; run 31283 confirmed the refusal of `stability_study/from180yrPk_viscRef_gmOn` without it. After this check the body applies the two build guards of the Build section (`nocheckpoint_switch_free=yes`, `approx_advection_implicit_vertical=yes`; the second keyed to the build record since 2026-09-12, to the `approxAdv` build before).
- **The adjoint-mode viscosity configuration is a build *and* a namelist variant, not just a build.** Its two source directories are `code_tap/variants/adjointViscosity/` and `input_tap/variants/adjointViscosity/`; its build identity — `build_tapAdj_adjVisc.sh`, `build_tapAdj_adjVisc/`, `run_token=tapAdj_ckpAll_adjVisc` — is the same word shortened, since 2026-09-05 (see the variant bullet above). It runs the adjoint with larger viscosity/diffusivity than the forward (`viscFacInAd = 10.` vs `viscFacInFw = 1.`), intended to keep a long adjoint from blowing up. **Before the 2026-08-31 mode-switch hooks it was inert under Tapenade**: the only routine applying those parameters (`ADAUTODIFF_INADMODE_SET`) is TAF-named and was never called, so every earlier "adjVisc" configuration silently ran plain physics. The boost engages only in builds carrying the Tapenade mode-switch hooks (`AUTODIFF_INADMODE_SET/UNSET`, named `TAP_INADMODE_*` until 2026-09-02; see "The ADJ* dump hook" above). Validated 2026-08-31: with hooks + default parameters, run 31024 reproduces 31023 bitwise; with hooks + the adjVisc pairing, run 31025 vs plain 31026 (same from-rest config) keeps `fc` bit-identical (the boost never touches the forward trajectory) while every nonzero `adxx_*`/`ADJ*` differs and peak sensitivities are damped — the first functioning adjVisc run in this project. `submit_tapAdj_adjVisc.sh` points `BUILD_DIR` at `build_tapAdj_adjVisc/` and, in its `stage_extra`, inserts `input_tap/variants/adjointViscosity/data.autodiff_additions` (the `inAd*`/`outAd*` values and `viscFacInFw`/`viscFacInAd`) into the staged `data.autodiff` before the end of `&AUTODIFF_PARM01`, refusing a staged file that already sets one of those keys; so the boost runs with the adjoint-mode switches of the staged namelist (since 2026-09-12; 31286 against 31271 identical). Until 2026-09-12 it replaced the staged file with a whole copy, `data.autodiff_adjointViscosity`, whose switches had to be kept in step by hand. Pairing the plain submit script with the adjVisc build (or the reverse) is refused, because each submit definition names its build's run token (`EXPECT_RUN_TOKEN`, since 2026-09-05); before that it silently ran a mismatched configuration. **The adjVisc build does not carry the `-nocheckpoint` list, on purpose.** Tried 2026-09-02: run 31056 (boost + list, 30 d from rest) vs 31025 (boost, every call checkpointed) — `fc` and all 441 `%MON` lines byte-identical, but all 66 `ADJ*` dumps and all 8 real `adxx_*` gradients differ at order one (RMS ratio 0.3–0.9), whereas the plain pair is bitwise identical under the same list. In joint mode Tapenade re-runs each routine's primal inside the backward sweep *after* `AUTODIFF_INADMODE_SET_B` has boosted the viscosities, so the boost reaches every recomputed intermediate; in split mode those intermediates were taped during the forward sweep at forward viscosities and the boost reaches only what the `_BWD` code reads live — a weaker, different regularisation. So the boosted adjoint stays a `ckpAll` build (run token `tapAdj_ckpAll_adjVisc`, 13 min per 30 d instead of 9), and 31056 stays on scratch as the record; report in `analyses/DINO_1deg/adjoint/tapenade_profiling/compare_30d_adjViscBoost_run31025_vs_nocheckpoint_run31056.md`. The exact condition came on 2026-09-12 (under "Tapenade profiling and `-nocheckpoint`" above): that list split `dynamics` and `thermodynamics`, which are recorded before the boost acts and read what it changes. **What the switches reach, and a stale-value fix (2026-09-09):** the stock `viscFacInAd` multiplies *only* the `PARM05` `viscAh[D/Z]file` fields (`mom_calc_visc.F:509-511`) — a scalar `viscAh` or `viscAhGrid` is never multiplied, so the file-based viscosity is what makes the boost reachable, and it preserves DINO's A_h ∝ Δx structure; the ASTE `inAdviscAhGrid` term is added in `MOM_CALC_VISC`, which runs only because the files set `useVariableVisc` at init (`set_parms.F:132`); `inAdviscA4Grid` is inert (`useBiharmonicVisc` fixed `.FALSE.` at init); `inAdviscArNr` acts; no `inAd*` scalar can reach the vertical diffusivity, which under `ALLOW_3D_DIFFKR` is the 3-D `diffKr` array. The `outAd*` values are what every checkpoint replay of the forward runs with after the first backward step and must equal the forward namelist's: until 2026-09-09 `outAdviscAhGrid=1.8E-2` (a `viscGrid1p8e-2` study value; production sets none) and `outAddiffKhT/S=0` (forward 500) were stale, so every boosted run up to 31138 replayed the forward with extra viscosity and no lateral tracer diffusion. Fixed in `data.autodiff_adjointViscosity` (whose boost lines are `data.autodiff_additions` since 2026-09-12); 31141 vs 31138 (30 d from rest, same executable) measures the change — DINO `TODO.md`.
...
- Namelist variants live in `input*/variants/<group>/` and are chosen by `test_cases` as `<group>/<tag>` (a bare `<tag>` still resolves to `variants/data_<tag>`; empty string = plain `input_tap/data`). A file is named for the MITgcm file it replaces, `<mitgcm-file>_<tag>`, and **every sibling sharing the tag is staged too** — so `stability_study/from180yrPk_viscRef_ReMax2_gmOn` brings its `data.pkg`, `data.autodiff` and `data.gmredi` along. The run directory takes the tag's last component only, never the group. See the setup README for the vocabulary.
...
**Run-directory names.** Every run directory is `<setup>_<run_token>_<duration>[_<tag>]_run<jobid>`, the token read from the build's `build_info.txt`: `frd` for a forward run (so forward names are unchanged from the older `<setup>_<mode>_…` form), `tapAdj_<ckp>[_<variant>]` for an adjoint (`<ckp>` is `nocheckpoint` or `ckpAll`; `<variant>` `adjVisc`, `profile` or `hooksInTree`), carried over from the 2026-09-02 renaming that took the token from the build record, and applied to SOMA as well since 2026-09-05. The `<tag>` uses the same configuration tokens the notebooks use (`visc2x`, `viscD2x_Zref`, `viscRef`, `viscGrid<v>`). **The job ID is the durable key** — settings tokens were derived by reading each run's own staged `data` namelist, so if a name and a namelist ever disagree, the namelist wins.
...
- `code_tap/packages.conf` adds `tapenade` plus the `adjoint` pkg group (`autodiff, ctrl, cost, grdchk`); in DINO the two lists are otherwise the same. **Neither DINO list compiles `kpp` or `cd_code` since 2026-09-12**, and each keeps both as commented lines whose notes say what turning them back on takes. KPP was off in every DINO namelist and no run on scratch used it; `code/` had compiled `cd_code` since 2026-05-01 only for the two forward variants that set `useCDscheme=.TRUE.` (`scheme_tests/from_rest_viscRef_CDscheme`, `viscosity_study/..._CDscheme`), the adjoint never had it, and all 42 DINO forward runs on scratch on 2026-09-12 log `pkg/cd_code compiled but not used`. Those two variants, `scheme_tests/from_rest_viscRef_kppON` and `input*/data.kpp` were deleted the same day (git history keeps them), and `useKPP`, `useMNC` and `useKPPinAdMode` are commented out with restore notes in `input*/data.pkg` and `input_tap/data.autodiff`: MITgcm stops at start-up when a `use<PKG>` flag is set for a package that is not compiled (`model/src/packages_check.F`). At the same time the forward `code/CPP_OPTIONS.h`, `GMREDI_OPTIONS.h` and `MOM_COMMON_OPTIONS.h` became copies of `code_tap/`'s (`ALLOW_SRCG` off, the sigma-coordinate code disabled, and GM/Redi without its Visbeck, boundary-value-problem, clipping, FM07, AC02 and submesoscale code, none of which the namelists select); the forward build without the two packages and with these headers reproduces the one before byte for byte (run 31280 against 31267, 61 d from rest: all 58 output files and every `%MON` line). **The C-D scheme computes the Coriolis terms from a second velocity carried on the D-grid** (`pkg/cd_code/cd_code_scheme.F`; MITgcm's `doc/getting_started/getting_started.rst`, "C-D Scheme") — not a velocity-smoothing filter, as the comments in `packages.conf` and the namelists said until 2026-09-12. **SOMA splits the other way, and there it is a real forward/adjoint difference:** both `code/` and `code_tap/` compile `cd_code`, but only `input_tap/data` sets `useCDscheme=.TRUE.`, so every SOMA adjoint run on scratch uses the scheme (31033, 31076, 31097, 31265) and every SOMA forward run does not (31034, 31096, 31274). Both start from rest, so no pickup crosses between them, but a SOMA forward run is not the trajectory its adjoint differentiates. The split came unchanged from the `v4_soma` setup SOMA was ported from; the SOMA README records it.
- `input_tap/` adds `data.autodiff`, `data.cost`, `data.ctrl`, `data.grdchk`.
- **`input*/` holds only what MITgcm reads; alternatives live in `input*/variants/`, grouped by purpose.** A submit script resolves `test_cases` to `variants/<group>/data_<tag>` (a bare `<tag>` still resolves to `variants/data_<tag>`; empty `test_cases` means the live `input*/data`) and stages only that one, so a run directory carries no unused namelists. **It also stages every sibling `<mitgcm-file>_<tag>` beside the chosen namelist** — that is how `stability_study/from180yrPk_viscRef_ReMax2_gmOn` gets its `data.pkg`, `data.autodiff` and `data.gmredi` as well as its `data`. Before 2026-08-28 only the `data` half was staged, so the variant that turned KPP on at the time (`scheme_tests/from_rest_viscRef_kppON`, deleted 2026-09-12) silently ran without KPP. The run directory is named after the tag alone, never the group. Anything placed directly in `input*/` is copied into *every* run. The staging uses `find -maxdepth 1 -type f` rather than a glob, because `cp dir/*` would hit `variants/` and abort under `set -e`.
- `code_tap/COST_OPTIONS.h` defines `ALLOW_COST_ATLANTIC_HEAT` and `ALLOW_COST_ATLANTIC_HEAT_DOMASS`.
- **Mixing coefficients: which are files and which are parameters (since 2026-09-09).** Vertical diffusivity is a namelist scalar, `diffKrT = diffKrS = 1.2E-5` (DINO's `rn_avt0`); under `ALLOW_3D_DIFFKR` it initialises the 3-D `diffKr` array (`ini_mixing.F`, from `diffKrNrS(k)`) that `xx_diffkr` perturbs, bitwise the same array the retired `diffKrFile='dino_diffKr.bin'` produced (31139 ≡ 31100 forward, 31140 ≡ 31137 adjoint, 30 d). The `kappa_v_ensemble` members set their κ the same way. Lateral viscosity **stays a `PARM05` file on purpose**: DINO's law is A_h = ½·U_v·Δx (NEMO `nn_ahm_ijk_t=20`, `rn_Uv=0.27`) and MITgcm has no parameter linear in Δx (`viscAhGrid` is ∝ L²/Δt, `viscAhReMax` is flow-dependent); `DINO_1deg/scripts/gen_viscAhD.py` regenerates every `dino_viscAhD*.bin` byte for byte from the grid file. The `_2p00` file, `rn_Uv=0.54`, was the production setting until 2026-09-09; **since 2026-09-10 production is the reference file plus `viscAhReMax=2.` with the flux-limited scheme 33 in the forward model and, through `useApproxAdvectionInAdMode` in the live `input_tap/data.autodiff`, the unlimited scheme 30 in the adjoint sweep** (honoured by the `approxAdv` build from 2026-09-10 and by every DINO adjoint build since 2026-09-12; from 2026-09-09 to 2026-09-10 it was scheme 30 in both; the DINO README's "Scheme 30 or scheme 33" has the study). The baseline is the live `input/data` (200 yr from rest) and `input_tap/data` (5 yr from a year-180 pickup), which the submit scripts run when `IMPACTS_TEST_CASE` is unset (`baseline/` keeps the `visc2x` predecessors as records); the kappa ensemble under this configuration is `kappa_v_ensemble/REF_ReMax2` and `M<k>_ReMax2`; the run-naming rule appends `_ReMax<v>` and `_adv<n>` (no token for scheme 33), since 2026-09-11 `_gmFwd` when `data.pkg` turns GM/Redi on while `data.autodiff` keeps it out of the adjoint sweep, and since 2026-09-12 `_approxAdv` when `data.autodiff` replaces scheme 33 in the adjoint sweep (from 2026-09-10 the build token `tapAdj_ckpAll_approxAdv` said so instead). **Vertical tracer advection is explicit since 2026-09-10** (`tempImplVertAdv = saltImplVertAdv = .FALSE.`): the implicit flux-limited DST3 solve returns a ±100 K checkerboard in a convectively homogenised column and blew up the 8× and 16× kappa legs within two years (31191/31193, dissected in 31198–31201; the DINO README's "Scheme 30 or scheme 33" has the account). The campaign was resubmitted under the corrected namelists: spin-up 31203, reference leg 31205 with the production adjoint 31206 chained on it, 31204 chained on the spin-up (cancelled on 2026-09-11, when GM/Redi in the adjoint's forward sweep was adopted; its replacement is on the DINO `TODO.md`) and the ensemble 31207–31220. The setup README's "Lateral viscosity and vertical diffusivity: file or parameter" has the table.
...
first of the three patches in `mods_tapenade_hooks/patches/`. SOMA is
