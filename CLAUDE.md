...
Both setups name scripts action-first (`build_*`, `submit_*`), and the `rawTapenade` control builds are retired in both — raw Tapenade output *is* the working configuration. **In DINO, since 2026-09-02, `scripts/build_tapAdj.sh` and `scripts/submit_tapAdj.sh` are symlinks** to the default adjoint pair beside them (`_approxAdv` since 2026-09-10, `_nocheckpoint` before); every real DINO adjoint script carries a token saying what it builds:
...
| `build_tapAdj_approxAdv.sh` / `submit_tapAdj_approxAdv.sh` | `build_tapAdj_approxAdv/` | **the default since 2026-09-10.** `ckpAll` **plus** `code_tap/variants/approxAdvection/` as a second `-mods` directory (since 2026-09-09): `gad_implicit_r.F` with the `useApproxAdvectionInAdMode` swap added for the implicit vertical advection, which the tree does not cover under any AD tool. Until 2026-09-12 the directory also held `gad_advection.F` with the block's guard widened from `ALLOW_AUTODIFF_TAMC` to `ALLOW_AUTODIFF`; that change is now in `mods_tapenade_hooks/`, so the `ckpAll` build honours the switch too and, with explicit vertical advection, gives the same adjoint. Scheme 33 in the forward sweep, scheme 30 in the adjoint sweep when `data.autodiff` sets the switch; every call checkpointed (the `_nocheckpoint` build takes the same switches since its list of 2026-09-12, but has no implicit-vertical swap). With the switch off it reproduces the `ckpAll` adjoint digit for digit (31179 vs 31178) |
...
| `build_tapAdj_nocheckpoint.sh` / `submit_tapAdj_nocheckpoint.sh` | **the default adjoint from 2026-09-02 to 2026-09-10** (`build_tapAdj.sh` / `submit_tapAdj.sh` were symlinks to this pair; the checkpoint-everything build lives on as `build_tapAdj_ckpAll.sh`). `-tap_extra "-nocheckpoint \"<list>\""`, the list read from `code_tap/tap_nocheckpoint.txt` (28 routines since 2026-09-12, each annotated with the gain that put it there). Tapenade differentiates those in split `_FWD`/`_BWD` mode instead of checkpointing them; the build fails unless every listed name produced a `_FWD` routine, so the list cannot silently drift, and records whether the list is safe under the adjoint-mode switches (`nocheckpoint_switch_free=`). |
