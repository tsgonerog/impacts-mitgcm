...
| `build_tapAdj_nocheckpoint.sh` / `submit_tapAdj_nocheckpoint.sh` | `build_tapAdj_nocheckpoint/` | profile-guided `-nocheckpoint` list (33 routines in split mode); bitwise identical to `ckpAll`, 1.5× faster. **The default from 2026-09-02 to 2026-09-10**; its list splits `gad_advection`, where the approximate-advection switch cannot act, so since 2026-09-12 the submit body refuses it whenever `data.autodiff` sets that switch, as it has refused the live GM-in-forward-sweep namelist since 2026-09-11 (split mode applies that switch to only half of the recomputation); use it with a switch-free `data.autodiff` as a deliberate exact-adjoint control |
