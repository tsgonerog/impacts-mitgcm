...
added, and `check_against_tree.sh` verifies that shape and derives the patch
series in `patches/` from it.
...
| `dummy_tap.F` | `pkg/tapenade/dummy_tap.F` | modify an existing file: additions only (the hook bodies appended after the four stock stubs) | +1024 |
...
mirroring `eesupp/src/exch_3d_rl.F` and its siblings. This is the one file in
the directory that removes lines from its tree counterpart: the stub bodies.
...
### `dummy_tap.F`: modify, additions only

The tree ships this file with four passive stubs (`DUMMY_IN_STEPPING_B`/`_D`,
`DUMMY_FOR_ETAN_B`/`_D`), which stay exactly as they are. Appended after them
are the hand-written derivatives, 37 routines: two helpers, and for each of
the seven externals (four field-shape hooks, the `etaN` hook, the two mode
switches) the forward no-op and its `_B` (reverse sweep: fold the halo with
`ADEXCH_*`, then `DUMP_ADJ_*`, or call `ADAUTODIFF_INADMODE_SET`/`UNSET`),
`_D` (tangent-linear: `WRITE_FLD_*` of the `G_J*` file) and `_FWD`/`_BWD`
(the pair a caller differentiated in split mode uses). The option headers and
`AD_CONFIG.h` the bodies need are included at the start of the appended block;
`CPP_EEOPTIONS.h`, which the stock file includes first, is guarded against
re-inclusion, so the stock prefix is untouched. The adjoint arrays arrive as
...
are; the `_TAP` suffix of the new routine names; and whether the
